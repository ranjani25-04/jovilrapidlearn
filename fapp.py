...
def generate_simple_certificate_pdf_bytes(student_name: str, course_name: str) -> bytes:
    ...
    lines = [
        "JovilRapidLearn Certificate",
        f"This certificate is awarded to {student_name}",
        f"For successfully completing the course: {course_name}",
        f"Issued on: {issued}",
    ]

    y = 700
    stream = "BT\n/F1 28 Tf\n72 {} Td\n({}) Tj\nET\n".format(y, esc(lines[0]))
    y -= 70
    stream += "BT\n/F1 16 Tf\n72 {} Td\n({}) Tj\nET\n".format(y, esc(lines[1]))
    y -= 30
    stream += "BT\n/F1 16 Tf\n72 {} Td\n({}) Tj\nET\n".format(y, esc(lines[2]))
    y -= 40
    stream += "BT\n/F1 12 Tf\n72 {} Td\n({}) Tj\nET\n".format(y, esc(lines[3]))
